...
2. Run ``coala-bears-create`` as mentioned above in order to create the wrapper
   for our ``C++`` bear and hit ``Enter`` for the first question. You can skip
   this step and use the wrapper provided below.
3. Name the bear ``coalaCheckBear`` and the executable ``coalaCheck_cpp``.
4. Reply to the rest of the questions (hit ``Enter`` at settings). After the
...
5. This tutorial will not focus on testing so ignore the second file for now.
   The wrapper should look something like this (after some cleaning):
...
6. Since the input will be a JSON string some kind of JSON class is needed.
   `This one <https://github.com/nlohmann/json>`__ is a great choice because it
   is easy to integrate and is used in this tutorial.
...
10. Read about the JSON Spec that the input uses(`The JSON Spec`_). The filename
    is found in ``in["filename"]`` and the list of lines is found in
    ``in["file"]``.
...
nlohmann's JSON lib makes it as easy as possible.
...
After building the executable it has to be added it to the ``PATH`` env
variable. It is possible to modify the wrapper and give there the full path.

13. Add the current directory to the ``PATH`` like so:
...
`testfile <https://github.com/Redridge/coalaCheckBear-cpp>`__).
...
uses a script that needs another binary to run (ex: python, bash, node).
...
argument is added then a comma has to be used at the end (ex ``(one_item,)``).
